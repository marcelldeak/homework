...
public class MachineFacade extends ItemFacade<Machine> {
    ...
    public Machine getMachineVisitorRiding(Long id) {
        return entityManager.createQuery("select m from Machine m where m.visitorsRiding.id = :id", Machine.class)
                .setParameter("id", id).getSingleResult();
    }

    public List<Machine> getMachinesFromThemePark(Long id) {
        return entityManager.createQuery("select m from Machine m where m.ownerThemePark.id = :id", Machine.class)
